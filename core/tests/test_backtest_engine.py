...
from core.backtest_engine import _pattern_a_post_pass_filter, _quality_gate, context_filter, format_backtest_summary, run_backtest_from_candles, should_allow_pattern_b
...
def test_context_filter_pattern_a_v2_cases():
    assert context_filter(3, 1, 'LOW') is True
    assert context_filter(2, 1, 'LOW') is True
    assert context_filter(1, 5, 'HIGH') is True
    assert context_filter(1, 5, 'LOW') is False
    assert context_filter(0, 4, 'MID') is True
    assert context_filter(0, 3, 'HIGH') is False
    assert context_filter(1, -1, 'HIGH') is False
    assert context_filter(0, 6, None) is False


def test_quality_gate_restores_baseline_weak_context_for_pattern_a():
    snapshot = {
        'context_label': 'WEAK',
        'context_score': 1,
        'bias_score': 5,
    ...
    ok, reason = _quality_gate(snapshot, 'LONG', 'RECLAIM')

    assert ok is False
    assert reason == 'WEAK_CONTEXT'


def test_quality_gate_restores_baseline_weak_context_for_flip_confirm():
    snapshot = {
        'context_label': 'WEAK',
        'context_score': 1,
        'bias_score': 5,
    ...
    ok, reason = _quality_gate(snapshot, 'LONG', 'FLIP_CONFIRM')
...
def test_pattern_a_post_pass_filter_blocks_low_context_after_pass():
    snapshot = {
        'context_score': 1,
        'bias_score': 5,
        'session_strength': 'LOW',
    ...
    ok, reason = _pattern_a_post_pass_filter(snapshot, entry_source='RECLAIM', quality_reason=None)

    assert ok is False
    assert reason == 'PATTERN_A_WEAK_CONTEXT_FILTER'


def test_pattern_a_post_pass_filter_skips_pattern_b2():
    snapshot = {
        'context_score': 1,
        'bias_score': 5,
        'session_strength': 'LOW',
    ...
    ok, reason = _pattern_a_post_pass_filter(snapshot, entry_source='PRESSURE_FLIP_ARM', quality_reason='PATTERN_B')

    assert ok is True
    assert reason is None

def test_quality_gate_keeps_pattern_b_only_for_pressure_flip_arm():
    snapshot = {
        'context_label': 'WEAK',
        'context_score': 1,
        'bias_score': 3,
    ...
    ok, reason = _quality_gate(snapshot, 'LONG', 'PRESSURE_FLIP_ARM')
...
def test_should_allow_pattern_b_boundary_cases():
    base_signal = {
        ...
        'diagnostic_context': {
            ...
            'bias_score': 3,
            'entry_side': 'LONG',
            'entry_source': 'PRESSURE_FLIP_ARM',
            'trigger_type': 'NONE',
        },
    }
    ...
    weak_context = {
        **base_signal,
        'diagnostic_context': {**base_signal['diagnostic_context'], 'context_score': 1},
    }
    weak_bias = {
        **base_signal,
        'diagnostic_context': {**base_signal['diagnostic_context'], 'bias_score': 2},
    }
    reclaim_trigger = {
        **base_signal,
        'trigger_type': 'RECLAIM',
        'diagnostic_context': {**base_signal['diagnostic_context'], 'trigger_type': 'RECLAIM'},
    }
    reclaim_trigger_negative_bias = {
        **base_signal,
        'trigger_type': 'RECLAIM',
        'diagnostic_context': {**base_signal['diagnostic_context'], 'trigger_type': 'RECLAIM', 'bias_score': -5},
    }

    assert should_allow_pattern_b(weak_context) is False
    assert should_allow_pattern_b(weak_bias) is False
    assert should_allow_pattern_b(reclaim_trigger) is False
    assert should_allow_pattern_b(reclaim_trigger_negative_bias) is False
    assert should_allow_pattern_b(base_signal) is True




def test_quality_gate_blocks_pattern_b_reclaim_filter():
    snapshot = {
        'context_label': 'VALID',
        'context_score': 2,
        'bias_score': 3,
    ...
    assert ok is False
    assert reason == 'PATTERN_B_RECLAIM_FILTER'


def test_backtest_pattern_b_allows_flip_edge_and_tags_trade(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    candles = _candles(140)
    calls = {'n': 0}
    ...
    assert result['trades'] == 1
    assert result['trades_data'][0]['action'] == 'PRESSURE_FLIP_ARM'
    assert result['trades_data'][0]['pattern'] == 'B2'
