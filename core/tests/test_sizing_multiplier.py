...
def test_markdown_yellow_strength7_strong_wr():
    """Base 1.0 + strength7 +0.2 = 1.2 × 1.1 (WR 65 ≥10) = 1.32 → 1.3"""
...
def test_markup_qualitative():
    """MARKUP 1h qualitative (per matrix) → base 0.4."""
...
def test_wr_below_threshold_neutral():
    """WR 80% but only 9 decided → multiplier = 1.0 (insufficient)"""
    d = compute_sizing("MARKDOWN", _fc("numeric", 0.40, 0.20),
                       _setup("short", 7), _wr(80, 9))
...
def test_wr_high_boosts():
    """WR 70% with 15 decided → × 1.1"""
    d = compute_sizing("MARKDOWN", _fc("numeric", 0.40, 0.20),
                       _setup("short", 7), _wr(70, 15))
    # 1.6 × 1.1 = 1.76 → 1.8
    assert d.multiplier == 1.8


def test_wr_low_penalizes():
    """WR 30% with 12 decided → × 0.7"""
    d = compute_sizing("MARKDOWN", _fc("numeric", 0.40, 0.20),
                       _setup("short", 7), _wr(30, 12))
    # 1.6 × 0.7 = 1.12 → 1.1
...
def test_direction_conflict_caps_at_0_5():
    """forecast=long bias, setup=short → cap pre at 0.5"""
    d = compute_sizing(
        "MARKDOWN", _fc("numeric", 0.62, 0.20),  # prob_up=0.62 → long bias
        _setup("short", 9), _wr(70, 15),
    )
    # base 1.4 + 0.4 = 1.8, but conflict caps at 0.5; × 1.1 = 0.55 → 0.6 (rounded)
    # Wait — round happens at end, after WR mult. 0.5 * 1.1 = 0.55 → round to 0.6
    assert d.multiplier == 0.6
...
def test_worked_example_1_strong_markdown():
    """Example 1: MARKDOWN green + SHORT strength=8 + WR 65/12 → 1.8"""
    d = compute_sizing(
        regime="MARKDOWN",
...
def test_worked_example_2_markup_qualitative_trap():
    """Example 2: MARKUP qual + LONG strength=7 + WR 50/10 → 0.6"""
    d = compute_sizing(
        regime="MARKUP",
...
def test_worked_example_3_direction_conflict():
    """Example 3: MARKDOWN green + LONG forecast bias + SHORT strength=9 → cap 0.5"""
    d = compute_sizing(
        regime="MARKDOWN",
    ...
    # Spec example said "0.5" but that was pre-WR; with WR ×1.1 it ends 0.6
    # Implementation follows spec literally (cap pre-WR at 0.5, then WR mult applies).
