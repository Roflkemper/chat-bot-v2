...
def _build_recent_closed_summary(trade_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    recent = closed[-20:]
    if not recent:
        return {'closed_trades_recent': 0, 'recent_winrate': 0.0, 'recent_avg_rr': 0.0, 'recent_avg_result_pct': 0.0, 'recent_streak_type': 'NONE', 'recent_loss_streak': 0, 'recent_win_streak': 0, 'recent_top_exit_reason': 'нет данных'}
    ...
    for row in reversed(recent):
        is_win = _extract_result_metrics(row)[2]
        row_type = 'WIN' if is_win else 'LOSS'
        if streak_type == 'NONE':
    ...
    for row in recent:
        reason = str(row.get('exit_reason_classifier') or row.get('exit_reason') or row.get('close_reason') or 'нет данных')
        exit_reasons[reason] = exit_reasons.get(reason, 0) + 1
    top_exit_reason, _ = _top_bucket(exit_reasons)
    return {
        ...
        'recent_streak_type': streak_type,
        'recent_loss_streak': streak_len if streak_type == 'LOSS' else 0,
        'recent_win_streak': streak_len if streak_type == 'WIN' else 0,
        'recent_top_exit_reason': top_exit_reason,
    }
...
def build_setup_learning_adjustment(setup_stats: Dict[str, Any], analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _safe_int(setup_stats.get('recent_loss_streak')) >= 3:
        delta -= 0.02
        reasons.append('серия убыточных выходов усиливает осторожность')
