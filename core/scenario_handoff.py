...
def compute_scenario_weights(snapshot: Dict[str, Any]) -> Tuple[int, int, List[str], List[str]]:
    ...
    if pressure == 'AGAINST':
        ...
        base_prob -= delta
        alt_reasons.append(f'давление локальных ТФ против блока ({pressure_strength.lower()})')
    ...
    if bars_at_edge >= 4 and not absorption_active:
        if consensus_side in {'LONG', 'SHORT'} and consensus_side != active_block:
            base_prob -= 10
            alt_reasons.append('4+ баров у края без absorption усиливают риск пробоя')
        else:
            base_prob -= 5
            alt_reasons.append('длительное давление у края повышает риск пробоя')
    ...
    if base_prob == 50:
        session_side = str((snapshot.get('forecast') or {}).get('session', {}).get('direction') or 'NEUTRAL').upper()
        if session_side in {'LONG', 'SHORT'} and session_side != active_block:
            base_prob -= 5
            alt_reasons.append('тай-брейкер: сессия поддерживает сценарий пробоя')
        elif scalp_dir == active_block:
            base_prob += 5
            base_reasons.append('тай-брейкер: скальп поддерживает отбой')
