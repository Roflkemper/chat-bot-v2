...
REPORT_DOW = 6  # Sunday (Mon=0..Sun=6)
REPORT_HOUR_UTC = 18
...
def should_send(now: datetime, *, state_path: Path = STATE_PATH) -> bool:
    ...
    try:
        ...
        return True
    # >= 6 days since last → новая неделя
    return (now - last_sent).total_seconds() >= 6 * 86400
...
def _peak_positions_week(*, snapshots: Path = SNAPSHOTS_CSV,
                         now: datetime,
        ...
        with snapshots.open(newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                ...
                if not bid:
                    continue
                try:
                    pos = abs(float(row.get("position") or 0.0))
...
def build_report(*,
    ...
    # 3. Risk-limit violations (peak > 100k за неделю)
...
def maybe_send_weekly(*,
    ...
    if not should_send(now, state_path=state_path):
        return False
    text = build_report(summary_fn=summary_fn, drift_summary_fn=drift_summary_fn,
                        now=now, snapshots=snapshots)
    try:
        send_fn(text)
    except Exception:
