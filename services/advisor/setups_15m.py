...
def _format_setup_line(s: dict) -> str:
    ...
    entry_s = fmt_price_compact(entry) if entry else "?"
    sl_s = fmt_price_compact(sl) if sl else "?"
    tp1_s = fmt_price_compact(tp1) if tp1 else "?"
    tp2_s = fmt_price_compact(tp2) if tp2 else "?"
    return (
