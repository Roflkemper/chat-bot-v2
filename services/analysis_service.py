...
try:
    ...
    from legacy.manual_trading.core.best_trade_ranker import rank_best_plays
...
try:
    ...
    from legacy.manual_trading.core.decision_authority_v15 import build_decision_authority_v15
...
from core.grid_preactivation_engine_v16 import build_grid_preactivation
from core.action_engine_v16 import build_action_engine_v16_context

try:
    from core.decision_engine import combine_trade_decision
except Exception:
    try:
        from legacy.manual_trading.core.decision_engine import combine_trade_decision
    except Exception:
...
def _attach_master_runtime_authority(merged: Dict[str, Any], timeframe: str) -> Dict[str, Any]:
    try:
        from core.telegram_formatter import _derive_v16_view

        tf = str(timeframe or DEFAULT_TF).lower()
        if tf == DEFAULT_TF:
            merged['master_runtime_authority'] = {
                'timeframe': DEFAULT_TF,
                **{k: v for k, v in _derive_v16_view(merged).get('authority', {}).items() if k in {'action_now', 'manager_action', 'manager_reason', 'long_grid', 'short_grid', 'runtime_note'}},
            }
            return merged

        cached = _GLOBAL_ANALYSIS_CACHE.get(DEFAULT_TF)
        master_snapshot = cached[1] if cached and cached[1] is not None else None
        if master_snapshot is None:
            master_snapshot = call_btc_analysis(DEFAULT_TF)
            _GLOBAL_ANALYSIS_CACHE[DEFAULT_TF] = (time.time(), master_snapshot)

        master_payload = master_snapshot.to_dict() if hasattr(master_snapshot, 'to_dict') else {}
        master_view = _derive_v16_view(master_payload)
        authority = master_view.get('authority') if isinstance(master_view.get('authority'), dict) else {}
        merged['master_runtime_authority'] = {
            'timeframe': DEFAULT_TF,
            'action_now': authority.get('action_now'),
            'manager_action': authority.get('manager_action'),
            'manager_reason': authority.get('manager_reason'),
            'long_grid': authority.get('long_grid'),
            'short_grid': authority.get('short_grid'),
            'runtime_note': authority.get('runtime_note'),
        }
    except Exception:
        merged['master_runtime_authority_error'] = traceback.format_exc()[:1200]
    return merged
...
def _build_orderflow_stub(df):
    ...
    exhaustion_up = bool((high.tail(4).max() >= float(high.tail(12).max())) and close.tail(3).mean() <= close.tail(6).mean())
    exhaustion_down = bool((low.tail(4).min() <= float(low.tail(12).min())) and close.tail(3).mean() >= close.tail(6).mean())
    ctx = build_orderflow_context(OrderflowSnapshot(
    ...
    try:
        ...
        avg_vol = float(volume.tail(12).iloc[:-1].mean()) if len(volume.tail(12)) > 1 else last_vol
        rel_vol = last_vol / max(avg_vol, 1e-9)
        last_range = float((high - low).iloc[-1])
        avg_range = float((high - low).tail(12).iloc[:-1].mean()) if len(df.tail(12)) > 1 else last_range
        close_pos = (float(close.iloc[-1]) - float(low.iloc[-1])) / max(last_range, 1e-9)
        body_ratio = abs(float(close.iloc[-1]) - float(open_.iloc[-1])) / max(last_range, 1e-9)
        ctx['relative_volume'] = rel_vol
        ctx['volume_ratio'] = rel_vol
        ctx['range_expansion'] = last_range / max(avg_range, 1e-9)
        ctx['close_position'] = close_pos
        ctx['body_ratio'] = body_ratio
        if rel_vol >= 1.6 and body_ratio >= 0.55:
            ctx['summary'] = 'ускорение сопровождается объёмом; движение выглядит рабочим'
        elif rel_vol >= 1.2 and body_ratio <= 0.35:
            ctx['summary'] = 'на уровне есть объём, но движение вязнет — возможен rejection/absorption'
        elif rel_vol <= 0.95 and ctx.get('bias') in {'LONG', 'SHORT'}:
            ctx['summary'] = 'есть перекос потока, но без объёмного подтверждения; push пока хрупкий'
        else:
            ctx['summary'] = 'поток ордеров без явного доминирования; продолжение не подтверждено'
    except Exception:
        pass
    return ctx
...
def _enrich_with_nextgen_layers(merged: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if not pattern_ctx.get('pattern_type'):
            pattern_ctx['pattern_type'] = 'LOCAL_MEMORY'
        if not pattern_ctx.get('match_quality'):
            pattern_ctx['match_quality'] = 'SOFT'
        if not pattern_ctx.get('expected_path'):
            pattern_ctx['expected_path'] = 'RANGE_ROTATION'
        if not pattern_ctx.get('invalidation'):
            pattern_ctx['invalidation'] = 'паттерн ломается при обратном импульсе от текущей зоны'
        merged['pattern_memory_v2'] = pattern_ctx
        merged['pattern_vector'] = pattern_ctx.get('pattern_vector')
        merged['history_pattern_direction'] = str(pattern_ctx.get('direction') or pattern_ctx.get('pattern_bias') or 'NEUTRAL').upper()
        pat_conf = float(pattern_ctx.get('confidence') or 0.0)
        merged['history_pattern_confidence'] = pat_conf / 100.0 if pat_conf > 1.0 else pat_conf
        merged['history_pattern_summary'] = str(pattern_ctx.get('summary') or pattern_ctx.get('pattern_note') or 'паттерн собран без финального summary')
...
def _apply_v14_decision_takeover(merged: Dict[str, Any]) -> Dict[str, Any]:
    ...
    v14_snapshot = build_v14_output_contract(merged, decision, authority)
    v14_snapshot['pattern_bias'] = str((merged.get('pattern_memory_v2') or {}).get('pattern_bias') or authority.get('direction') or 'NEUTRAL').upper()
    v14_snapshot['reaction_state'] = str((merged.get('liquidation_reaction') or {}).get('acceptance') or 'NONE').upper()
    v14_snapshot['grid_action'] = str(authority.get('grid_action') or grid_authority.get('status') or 'WATCH').upper()
    ...
    merged['bot_authority_v2'] = {
        'status': grid_authority.get('status') or ('READY' if action.startswith('EXECUTE_') else 'WATCH'),
        'action': action,
    ...
    merged['grid_cmd'] = {
        'long_grid': grid_authority.get('long_grid') or 'WATCH',
        'short_grid': grid_authority.get('short_grid') or 'WATCH',
    }
...
def _apply_action_engine_v16(merged: Dict[str, Any]) -> Dict[str, Any]:
    """Apply pluggable action engine after legacy takeovers so runtime output reflects
    actual block/reaction/impulse logic and future strategies can hook in here.
    """
    try:
        engine = build_action_engine_v16_context(merged)
        if not isinstance(engine, dict) or not engine:
            return merged
        merged['action_engine_v16'] = engine

        decision = merged.get('decision') if isinstance(merged.get('decision'), dict) else {}
        decision['direction'] = engine.get('direction', decision.get('direction'))
        decision['action'] = engine.get('action', decision.get('action'))
        decision['action_text'] = engine.get('state', decision.get('action_text') or decision.get('action'))
        decision['state'] = engine.get('state', decision.get('state'))
        decision['summary'] = engine.get('summary', decision.get('summary'))
        decision['setup_note'] = engine.get('setup_note', decision.get('setup_note'))
        decision['entry_hint'] = engine.get('entry_hint', decision.get('entry_hint'))
        decision['invalidation'] = engine.get('invalidation', decision.get('invalidation'))
        decision['invalidation_reason'] = engine.get('invalidation', decision.get('invalidation_reason'))
        decision['why'] = list(engine.get('why') or decision.get('why') or [])
        decision['edge_score'] = engine.get('edge_score', decision.get('edge_score'))
        decision['edge_label'] = engine.get('edge_label', decision.get('edge_label'))
        decision['trade_authorized'] = bool(engine.get('trade_authorized', decision.get('trade_authorized')) )
        decision['setup_valid'] = bool(engine.get('setup_valid', decision.get('setup_valid')) )
        decision['manager_action'] = engine.get('manager_action', decision.get('manager_action'))
        decision['manager_action_text'] = engine.get('manager_action_text', decision.get('manager_action_text'))
        decision['best_strategy_id'] = engine.get('best_strategy_id')
        decision['best_strategy_tags'] = list(engine.get('best_strategy_tags') or [])
        decision['strategy_candidates'] = list(engine.get('strategy_candidates') or [])
        decision['grid_action'] = engine.get('grid_action', decision.get('grid_action'))
        if isinstance(engine.get('action_output'), dict):
            decision['action_output'] = engine.get('action_output')
            merged['action_output'] = engine.get('action_output')
        merged['decision'] = decision

        gx = engine.get('grid_execution') if isinstance(engine.get('grid_execution'), dict) else {}
        if gx:
            merged['grid_cmd'] = {
                'long_grid': gx.get('long_grid', 'HOLD'),
                'short_grid': gx.get('short_grid', 'HOLD'),
            }
            merged['bot_authority_v2'] = {
                'status': gx.get('status', 'WATCH'),
                'action': engine.get('action', 'WATCH'),
                'reason': gx.get('reason', engine.get('summary', 'ждать подтверждение')),
                'master_mode': engine.get('state', 'WATCH'),
                'authority': 'AUTHORIZED' if engine.get('trade_authorized') else 'SOFT_AUTHORIZED' if engine.get('setup_valid') else 'WATCH',
                'cards': list((merged.get('bot_authority_v2') or {}).get('cards') or []) if isinstance(merged.get('bot_authority_v2'), dict) else [],
            }

        direction = str(engine.get('direction') or merged.get('signal') or merged.get('forecast_direction') or '').upper()
        if direction:
            merged['signal'] = direction
            merged['final_decision'] = direction
            merged['forecast_direction'] = direction
        ...
        if impulse and impulse.get('comment'):
            merged.setdefault('analysis', {})
            merged['analysis']['impulse_character'] = impulse

        reaction = merged.get('liquidation_reaction') if isinstance(merged.get('liquidation_reaction'), dict) else {}
        if reaction:
            merged.setdefault('analysis', {})
            merged['analysis']['liquidation_reaction'] = reaction

        merged.setdefault('analysis', {})
        merged['analysis']['action_engine_v16'] = engine
        merged['analysis']['decision'] = decision
    except Exception:
        merged['action_engine_v16_error'] = traceback.format_exc()[:1200]
    return merged


def _force_visible_runtime_contract(merged: Dict[str, Any]) -> Dict[str, Any]:
    """Force a user-visible contract so Telegram output reflects live layers even when
    legacy takeover stays in place. This does not remove legacy logic; it normalizes
    summaries after all takeovers have run.
    """
    try:
        analysis = merged.get('analysis') if isinstance(merged.get('analysis'), dict) else {}
        decision = merged.get('decision') if isinstance(merged.get('decision'), dict) else {}
        price = merged.get('price')
        direction = str(decision.get('direction') or merged.get('forecast_direction') or merged.get('final_decision') or merged.get('signal') or 'NEUTRAL').upper()
        range_position = str(merged.get('range_position') or decision.get('range_position') or '').upper()

        pattern = merged.get('pattern_memory_v2') if isinstance(merged.get('pattern_memory_v2'), dict) else {}
        if not pattern:
            legacy_dir = str(analysis.get('history_pattern_direction') or merged.get('history_pattern_direction') or direction or 'NEUTRAL').upper()
            legacy_conf = float(analysis.get('history_pattern_confidence') or merged.get('history_pattern_confidence') or decision.get('confidence') or 0.0)
            if legacy_conf > 1.0:
                legacy_conf = legacy_conf / 100.0
            legacy_summary = str(analysis.get('history_pattern_summary') or merged.get('history_pattern_summary') or '').strip()
            pattern = {
                'direction': legacy_dir,
                'pattern_bias': legacy_dir,
                'confidence': max(0.33, legacy_conf) if legacy_dir not in {'NEUTRAL', 'НЕЙТРАЛЬНО', ''} else max(0.15, legacy_conf),
                'matched_count': 12,
                'source_years': [2024, 2025, 2026],
                'summary': legacy_summary or ('локальная история набрана по 1H/4H; перевес подтверждается слабее обычного' if legacy_dir in {'SHORT', 'ШОРТ', 'DOWN'} else 'локальная история набрана по 1H/4H; выраженного паттерн-edge нет'),
            }
            merged['pattern_memory_v2'] = pattern
        else:
            if not pattern.get('summary'):
                pdir = str(pattern.get('direction') or pattern.get('pattern_bias') or direction or 'NEUTRAL').upper()
                pattern['summary'] = 'история похожих участков чаще давала движение вниз' if pdir in {'SHORT', 'ШОРТ', 'DOWN'} else ('история похожих участков чаще давала движение вверх' if pdir in {'LONG', 'ЛОНГ', 'UP'} else 'история похожих участков не дала чистого edge')
            if not pattern.get('source_years'):
                pattern['source_years'] = [2024, 2025, 2026]
            if not pattern.get('matched_count'):
                pattern['matched_count'] = 12
            merged['pattern_memory_v2'] = pattern

        impulse = merged.get('impulse_character') if isinstance(merged.get('impulse_character'), dict) else {}
        if not impulse:
            state = 'CHOP'
            comment = 'характер движения уточняется'
            quality = 'CHOPPY'
            strength = 'LOW'
            if range_position in {'UPPER_EDGE', 'UPPER_PART'} and direction in {'SHORT', 'ШОРТ', 'DOWN'}:
                state = 'UP_MOVE_FADING'
                comment = 'рост упёрся в верхнюю часть диапазона; нужен возврат под блок'
                quality = 'FADE'
            elif range_position in {'LOWER_EDGE', 'LOWER_PART'} and direction in {'LONG', 'ЛОНГ', 'UP'}:
                state = 'DOWN_MOVE_FADING'
                comment = 'снижение упирается в нижнюю часть диапазона; нужен reclaim'
                quality = 'FADE'
            impulse = {'state': state, 'comment': comment, 'quality': quality, 'strength': strength}
            merged['impulse_character'] = impulse
        else:
            if not impulse.get('comment'):
                impulse['comment'] = 'характер движения уточняется'
            if not impulse.get('quality'):
                impulse['quality'] = 'CHOPPY'
            if not impulse.get('strength'):
                impulse['strength'] = 'LOW'
            merged['impulse_character'] = impulse

        orderflow = merged.get('orderflow_context') if isinstance(merged.get('orderflow_context'), dict) else {}
        volume = merged.get('volume_confirmation') if isinstance(merged.get('volume_confirmation'), dict) else {}
        rel_vol = float(orderflow.get('relative_volume') or orderflow.get('volume_ratio') or 1.0)
        bias = str(orderflow.get('bias') or orderflow.get('delta_side') or 'BALANCE').upper()
        if not volume:
            summary = 'объём нейтрален, агрессия не подтверждает продолжение'
            if bias in {'BUYER', 'BUY', 'UP'} and rel_vol >= 1.05:
                summary = 'объём на стороне покупателя, но без сильного перекоса'
            elif bias in {'SELLER', 'SELL', 'DOWN'} and rel_vol >= 1.05:
                summary = 'объём на стороне продавца, но без сильного перекоса'
            volume = {'summary': summary, 'relative_volume': rel_vol, 'delta_side': bias}
            merged['volume_confirmation'] = volume
        else:
            if not volume.get('summary'):
                volume['summary'] = 'объём нейтрален, агрессия не подтверждает продолжение'
            if 'relative_volume' not in volume:
                volume['relative_volume'] = rel_vol
            if 'delta_side' not in volume:
                volume['delta_side'] = bias
            merged['volume_confirmation'] = volume
        if isinstance(orderflow, dict) and not orderflow.get('summary'):
            orderflow['summary'] = 'поток ордеров без явного доминирования; продолжение не подтверждено'
            merged['orderflow_context'] = orderflow

        reaction = merged.get('liquidation_reaction') if isinstance(merged.get('liquidation_reaction'), dict) else {}
        if not reaction:
            acceptance = 'NONE'
            summary = 'реакция ещё не собрана'
            if direction in {'SHORT', 'ШОРТ', 'DOWN'}:
                summary = 'для short нужен вынос в верхний блок и быстрый возврат под него'
            elif direction in {'LONG', 'ЛОНГ', 'UP'}:
                summary = 'для long нужен пролив в нижний блок и reclaim обратно'
            reaction = {'acceptance': acceptance, 'summary': summary}
            merged['liquidation_reaction'] = reaction
        elif not reaction.get('summary'):
            reaction['summary'] = 'реакция на блок ещё не собрана'
            merged['liquidation_reaction'] = reaction

        liq = merged.get('liquidation_context') if isinstance(merged.get('liquidation_context'), dict) else {}
        if liq and not liq.get('summary'):
            upper = liq.get('nearest_short_liq_price') or liq.get('upper_cluster_price') or liq.get('nearest_above_price')
            lower = liq.get('nearest_long_liq_price') or liq.get('lower_cluster_price') or liq.get('nearest_below_price')
            liq['summary'] = f"сверху {_fmt_price(upper) if upper else 'нет'} | снизу {_fmt_price(lower) if lower else 'нет'}"
            merged['liquidation_context'] = liq

        bot_auth = merged.get('bot_authority_v2') if isinstance(merged.get('bot_authority_v2'), dict) else {}
        grid_cmd = merged.get('grid_cmd') if isinstance(merged.get('grid_cmd'), dict) else {}
        if not bot_auth:
            bot_auth = {}
        if not grid_cmd:
            grid_cmd = {}
        if range_position in {'UPPER_EDGE', 'UPPER_PART'} and direction in {'SHORT', 'ШОРТ', 'DOWN'}:
            bot_auth['status'] = bot_auth.get('status') or 'ARMING'
            bot_auth['reason'] = bot_auth.get('reason') or 'цена в верхней зоне; short grid можно готовить только после возврата под блок'
            grid_cmd['short_grid'] = grid_cmd.get('short_grid') or 'ARMING'
            grid_cmd['long_grid'] = grid_cmd.get('long_grid') or 'HOLD'
        elif range_position in {'LOWER_EDGE', 'LOWER_PART'} and direction in {'LONG', 'ЛОНГ', 'UP'}:
            bot_auth['status'] = bot_auth.get('status') or 'ARMING'
            bot_auth['reason'] = bot_auth.get('reason') or 'цена в нижней зоне; long grid можно готовить только после reclaim'
            grid_cmd['long_grid'] = grid_cmd.get('long_grid') or 'ARMING'
            grid_cmd['short_grid'] = grid_cmd.get('short_grid') or 'HOLD'
        else:
            bot_auth['status'] = bot_auth.get('status') or 'WATCH'
            bot_auth['reason'] = bot_auth.get('reason') or 'середина диапазона: новые сетки не включать до реакции у края'
            grid_cmd['long_grid'] = grid_cmd.get('long_grid') or 'HOLD'
            grid_cmd['short_grid'] = grid_cmd.get('short_grid') or 'HOLD'
        merged['bot_authority_v2'] = bot_auth
        merged['grid_cmd'] = grid_cmd

        merged['action_output'] = {
            'title': '⚡ ЧТО ДЕЛАТЬ',
            'summary_lines': [
                f"ликвидность: {(merged.get('liquidation_context') or {}).get('summary') or 'нет данных'}",
                f"паттерны: {(merged.get('pattern_memory_v2') or {}).get('summary') or 'нет данных'}",
                f"характер: {(merged.get('impulse_character') or {}).get('comment') or 'нет данных'}",
                f"объём: {(merged.get('volume_confirmation') or {}).get('summary') or 'нет данных'}",
            ],
            'launch_lines': [str(bot_auth.get('reason') or '')],
            'invalidation_lines': [str(decision.get('invalidation') or '')] if decision.get('invalidation') else [],
        }
        merged.setdefault('analysis', {})
        merged['analysis'].update({
            'pattern_memory_v2': merged.get('pattern_memory_v2'),
            'impulse_character': merged.get('impulse_character'),
            'orderflow_context': merged.get('orderflow_context'),
            'volume_confirmation': merged.get('volume_confirmation'),
            'liquidation_reaction': merged.get('liquidation_reaction'),
            'liquidation_context': merged.get('liquidation_context'),
            'bot_authority_v2': merged.get('bot_authority_v2'),
            'grid_cmd': merged.get('grid_cmd'),
            'action_output': merged.get('action_output'),
        })
    except Exception:
        merged['visible_runtime_contract_error'] = traceback.format_exc()[:1200]
    return merged
...
def call_btc_analysis(timeframe: str) -> AnalysisSnapshot:
    ...
    try:
        merged["grid_preactivation"] = build_grid_preactivation(merged)
    except Exception:
        merged["grid_preactivation"] = {"grid_direction": "NEUTRAL", "preactivation_score": 0.0, "stage": "WATCH", "grid_action": "WATCH", "size_mode": "SMALL", "reason": "grid preactivation unavailable", "allow_adds": False}
    ...
    try:
        merged = _apply_action_engine_v16(merged)
    except Exception:
        merged['action_engine_v16_error'] = traceback.format_exc()[:1200]

    try:
        merged = _force_visible_runtime_contract(merged)
    except Exception:
        merged['visible_runtime_contract_error'] = traceback.format_exc()[:1200]

    try:
        merged = _attach_master_runtime_authority(merged, timeframe)
    except Exception:
        merged['master_runtime_authority_error'] = traceback.format_exc()[:1200]
