...
def fmt_price_compact(value) -> str:
    """Compact price formatter with magnitude-aware precision for TG alerts.

    BTC ($80123) → '80123', ETH ($3450.5) → '3450', XRP ($1.4032) → '1.4032',
    DOGE ($0.12) → '0.1200', micro ($0.00012) → '0.000120'.

    Was: hard-coded `:.0f` rounded XRP to '1' and TP1/TP2 collapsed to the
    same integer, making PAPER trade alerts read as 'LONG @ 1 | SL 1 | TP1 1'.
    """
    if value is None:
        return "?"
    try:
