"""B3: Combined vs baseline using ADAPTIVE thresholds (vol-scaled).

Same as B2 v2, but PauseEntries/PartialUnload thresholds scale with current
ATR volatility (intervention_rules_adaptive). Question: does adaptive
calibration improve combined-scenario win rate vs fixed thresholds?
...
OUT_MD = ROOT / "docs" / "STRATEGIES" / "COMBINED_ALL_BOTS_V3_ADAPTIVE.md"
OUT_CSV = ROOT / "state" / "combined_all_bots_v3_adaptive.csv"
...
def make_intervention_rules(scenario: str) -> list:
    from services.managed_grid_sim.intervention_rules import (
        ResumeEntriesOnPullback, RaiseBoundaryOnConfirmedTrend,
    )
    ...
    if scenario == "baseline":
        return []
    # AdaptivePause uses base_threshold_usd; convert -30% of typical depo $1000 -> -$300.
    # In low-vol the threshold tightens (-$150), in high-vol it widens (-$750).
    rules = [
        AdaptivePauseEntriesOnUnrealizedThreshold(
            base_threshold_usd=-300.0, hold_time_minutes=5),
        ResumeEntriesOnPullback(
            pullback_from_peak_pct=50.0, hold_minutes_after_peak=10),
        AdaptivePartialUnloadOnRetracement(
            base_unrealized_threshold_usd=50.0,
            base_retracement_pct=40.0, unload_fraction=0.3),
