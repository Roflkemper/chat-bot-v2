...
## §16 OPERATOR TRADING PROFILE

**Last update:** 2026-04-30  
**Status:** v0.1 — первая формализация, итеративное уточнение по мере анализа данных  
**Source:** наблюдения architect'а из переписки + CANON/STRATEGY_CANON_2026-04-30 + session 30.04

---

### §16.1 Trading philosophy

Цель: зарабатывать всеми возможными способами на BTC/XRP/ETH через portfolio of grid bots с активным management.

**Главное правило P0** — не закрывать в минус без крайней необходимости. Сетка должна вытащить через работу, не через ножницы.

**Главный edge** — активное управление поверх сетки: закрытия / перезапуски / boundary moves. Базовая сетка одна не справляется с направленным движением. Оператор видит momentum / exhaustion / liquidations и вмешивается.

---

### §16.2 Двухдвижковая архитектура

**LONG-side (USDT-M, linear):**
- Размер: USD-номинирован ($100-200 за ордер)
- PnL: в USD
- Поведение в росте: прямой directional gain + микро-TP grind
- Live боты: BTC-LONG-C, BTC-LONG-D

**SHORT-side (COIN-M, inverse):**
- Размер: BTC-номинирован (0.001-0.003 BTC за ордер)
- PnL: в BTC
- Поведение в росте: позиция накапливается, currency cushion частично компенсирует
- Live боты: TEST_1, TEST_2, TEST_3

**В боковике:** оба движка генерируют реализованный PnL независимо — double-sided grind.  
**В тренде:** одна сторона страдает, другая выигрывает + currency compensation.

Детали: `GINAREA_MECHANICS.md` §1-§2.

---

### §16.3 Активные вмешательства оператора

| Вмешательство | Частота | Триггер | Статус |
|---|---|---|---|
| Close LONG на impulse top, restart на откате | регулярно | exhaustion + proximity to cluster | confirmed (P-9 + manual restart) |
| Activate booster Bot 6399265299 | ситуативно | impulse исчерпан у liq, set hard border.top выше хая | confirmed (custom bot, P-16 кандидат) |
| Raise SHORT boundary | тактически | confirmed trend up + price held above resistance | confirmed (P-1) |
| Pause SHORT new entries | защитно | trend up 3h+ без отката | confirmed (P-4) |
| Stack-bot on pullback | оппортунистически | rally 2-3% → потеря momentum | confirmed (P-2) |
| Stack-long after dump | оппортунистически | dump >2% + reversal signal | confirmed (P-7) |

Полные условия — PLAYBOOK.md.

---

### §16.4 Паттерны которые оператор избегает

По данным OPPORTUNITY_MAP_v1.md:

| Паттерн | Avg impact | Почему вреден |
|---|---|---|
| P-5 partial unload | -$26 | Фиксирует убыток, market recovers |
| P-8 force close + restart | -$192 | Теряется позиция которая выйдет сама |
| P-10 rebalance close + reenter | -$46 | Платишь spread дважды, позиция хуже |
| Force close в минус | P0 violation | Сетка вытащит при терпении |

---

### §16.5 Sizing rules

Три режима из OPPORTUNITY_MAP_v1.md §4 (депозит ~$15k):

| Режим | Size | Когда |
|---|---|---|
| conservative | 0.05 BTC | DD > 5%, free margin < 30% |
| normal (default) | 0.10 BTC | Стандарт, margin 30-60% |
| aggressive | 0.18 BTC | Free margin > 60%, нет позиций в DD |

---

### §16.6 Интуитивные решения оператора — gaps для data analysis

Что оператор делает по ощущениям и что нужно превратить в quantifiable rules:

**Gap 1 — Когда именно "impulse exhausted"?**

Оператор активирует Booster Bot 6399265299 "по ощущению исчерпанности".  
Нужно: ATR drop + volume divergence + candles range + RSI divergence.  
→ TZ-BOOSTER-ACTIVATION-PATTERNS: читать ginarea tracker history, найти timestamps активации бота, 30-60 свечей context. Output: `docs/analysis/BOOSTER_ACTIVATION_PATTERNS.md`.

**Gap 2 — Close LONG на top vs hold?**

Оператор закрывает лонг "когда кажется что вот-вот откат". Иногда угадывает, иногда закрыл рано.  
Известные эпизоды: 16.04 ~15:10 (закрыл + re-entry -0.9%), 17.04 утро (не закрыл — выжил), 17.04 каскад (закрыл + re-entry -0.9%).  
→ TZ-MANUAL-LONG-CLOSE-ANALYSIS: для каждого эпизода — context before close, outcome 1ч/4ч, counterfactual hold. Output: `docs/analysis/MANUAL_LONG_CLOSES_2026-04-16-17.md`.

**Gap 3 — Raise boundary vs pause vs do nothing?**

Три защитных реакции на uptrend. Выбор по feel.  
Decision tree в разработке — нужны: continuation strength (delta_4h, ATR, OI), position health (unrealized %, liq distance), time in drawdown.  
→ Phase 2 после накопления paper_journal data.

**Gap 4 — Multi-asset coordination?**

BTC trend up + XRP не следует → BTC откатится? XRP impulse solo → fade signal?  
→ /advise уже multi-asset (28.04). Расширить: cross-asset divergence detection. Future work.

---

### §16.7 Path to autonomy

**Phase 1 (running, старт 28.04):** paper_journal пишет /advise рекомендации + outcomes 14 дней. Цель: показать что /advise predictions useful.

**Phase 2 (после Phase 1):** operator decisions tracker. Оператор делает/не делает → tracker записывает → comparison с /advise. Закрывает Gap 1-3 через данные.
...
```
2026-04-30 v1.1 — Добавлен §16 OPERATOR TRADING PROFILE (v0.1).
                  Источники: CANON/STRATEGY_CANON_2026-04-30, session 30.04 conversation,
                  OPPORTUNITY_MAP_v1, PLAYBOOK. Первая формализация trading profile.
