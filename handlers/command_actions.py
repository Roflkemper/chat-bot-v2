    ...
    def margin(self) -> BotResponsePayload:
        ...
        if len(parts) <= 1:
            ...
            if current is not None:
                ...
                lines.append("")
            lines.append("Чтобы обновить — 3 числа в любом формате:")
            lines.append("  /margin 96 21000 18")
            lines.append("  /margin 0.96 21k 18%")
            lines.append("  /margin <95.56> <$21,301> <14>")
            lines.append("")
            lines.append("Порядок: coef → available → dist_to_liq")
            lines.append("Парсер сам понимает % $ <> запятые тысячи k=1000")
            lines.append("96 = 0.96 автоматически")
            lines.append("")
            lines.append("Где взять на BitMEX:")
            lines.append("  • coefficient — 'Состояние' в правом верхнем углу (например 95.56%)")
            lines.append("  • available — 'Доступно' в правом блоке (USD)")
            lines.append("  • dist_to_liq — % дистанции до liq до самой ближней позиции")
            return self.ctx.plain("\n".join(lines))
