...
def build_debug_keyboard() -> ReplyKeyboardMarkup:
    ...
    kb.row(_btn("⚡ ЧТО ДЕЛАТЬ СЕЙЧАС"), _btn("СТАТУС БОТОВ"), _btn("МЕНЕДЖЕР BTC"))
    kb.row(_btn("МОЯ ПОЗИЦИЯ"), _btn("BTC INVALIDATION"), _btn("ПОМОЩЬ"))
...
def build_dynamic_keyboard(state: Dict[str, Any] | None = None) -> ReplyKeyboardMarkup:
    ...
    if has_position:
        kb.row(_btn("МЕНЕДЖЕР BTC"), _btn("МОЯ ПОЗИЦИЯ"), _btn("BTC INVALIDATION"))
        ...
        if risk == "HIGH" or action in {"WAIT", "ЖДАТЬ", "NO_TRADE"}:
            kb.row(_btn("ПОЧЕМУ НЕТ СДЕЛКИ"), _btn("BTC INVALIDATION"))
